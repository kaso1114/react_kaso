...
function AdminProducts() {
  ...
  const getProductData = async (page = 1) => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE}/api/${apiPath}/admin/products`,
        {
          params: { page },
        }
      );
  ...
  const openModal = (product, type) => {
    if (type === "create") {
      ...
      const imagesUrl = Array.isArray(product?.imagesUrl)
        ? [...product.imagesUrl]
        : [];
  ...
  const handleSubmit = async () => {
    if (modalType === "delete") {
      try {
        await axios.delete(
          `${API_BASE}/api/${apiPath}/admin/product/${templateData.id}`
        );
        closeModal();
        await getProductData();
      } catch (error) {
    ...
    try {
      if (modalType === "create") {
        await axios.post(`${API_BASE}/api/${apiPath}/admin/product`, {
          data: payload,
        });
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE}/api/${apiPath}/admin/upload`,
        formData
      );
                  ...
                  {products.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="text-center">
                        <span
                          className={item.is_enabled ? "text-success" : "text-muted"}
                        >
